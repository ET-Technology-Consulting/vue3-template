    ...
    <table class="table-fixed min-w-full divide-y divide-secondary-300 dark:divide-secondary-700">
      <thead class="bg-secondary-100 dark:bg-secondary-800">
        <tr>
          ...
          <!-- <th scope="col"
            class="py-3.5 pl-4 pr-3 text-right text-sm font-semibold text-secondary-900 dark:text-secondary-100 sm:pl-6">Action</th> -->
        </tr>
      </thead>
      <tbody
      ...
      >
        ...
        >
          <td
            ...
            @click="$router.push(`/dashboard/backtrack/${d.id}`)"
          >
            {{ d.id }}
          </td>
          <td
          ...
          >
            <span
              v-for="p in d.media_ids.split(',')"
              class="inline-flex items-center rounded-full bg-primary-50 px-2 py-1 mx-1 text-xs font-medium text-primary-700 ring-1 ring-inset ring-primary-600/20"
              >{{ p }}</span
            >
          </td>
          <td
          ...
          <!-- <td class="py-4 pl-4 pr-3 text-right">
            <span class="isolate inline-flex rounded-md shadow-sm">
              <button type="button" :class="[
                'relative inline-flex items-center rounded-l-md px-3 py-2 text-sm font-semibold ring-1 ring-inset ring-secondary-300 hover:bg-secondary-500 focus:z-10',
                d.is_active === true ? 'bg-primary-500 text-secondary-50' : 'bg-white text-secondary-900',
              ]" @click="$emit('edit', d.id)">Edit</button>
            </span>
          </td> -->
        </tr>
      </tbody>
